...
puts "1"
file = URI.open("https://upload.wikimedia.org/wikipedia/commons/2/2b/Ickworth_Park%2C_Suffolk%2C_England_-broken_bench-2March2012.jpg")
...
report = Report.create!(title:"Dangerous pothole", description: "There is a dangerous pothole on Rambla del Raval that already lead to several accidents. It should be fixed as soon as possible!", location: "Rambla del Raval 4, Barcelona", user: User.all.sample, category: Category.all[3])
...
report = Report.create!(title:"Piles of trash", description: "The trash cans are always full and never get emptied. It attracts rats and other animals and smells horrible! This has to change.", location: "Carrer de l'Hospital 46, Barcelona", user: User.all.sample, category: Category.all[8])
report.photo.attach(io: file, filename: "trash.jpg", content_type: 'image/jpg')
puts "4"
file = URI.open("https://upload.wikimedia.org/wikipedia/commons/thumb/b/bd/Street_lamp%2C_Edinburgh_New_Town.jpg/1200px-Street_lamp%2C_Edinburgh_New_Town.jpg")
...
puts "5"
file = URI.open("https://upload.wikimedia.org/wikipedia/commons/7/75/Great_Portland_Street_London_Traffic.jpg")
report = Report.create!(title:"Noisy traffic", description: "The expansion of Carrer de la Riera Alta has lead to serious noise issues for residents. We can't sleep at night anymore and the pollution is horrible.", location: "Carrer de la Riera Alta 23, Barcelona", user: User.all.sample, category: Category.all[9])
report.photo.attach(io: file, filename: "traffic.jpg", content_type: 'image/jpg')
puts "6"
file = URI.open("https://arc-anglerfish-arc2-prod-tbt.s3.amazonaws.com/public/4ABN7POFTFHENBPKDI4QYDLBQY.jpg")
report = Report.create!(title:"Sewage leak", description: "There has been a sewage leak on Carrer de Jaume I. It needs to be fixed immediately, the smell is disgusting and drives out the tourists.", location: "Carrer de Jaume I 14, Barcelona", user: User.all.sample, category: Category.all[1])
report.photo.attach(io: file, filename: "sewage.jpg", content_type: 'image/jpg')
puts "7"
file = URI.open("https://3kpnuxym9k04c8ilz2quku1czd-wpengine.netdna-ssl.com/wp-content/uploads/2019/05/42615859735_d3100ec492_o.jpg")
report = Report.create!(title:"Plants need watering", description: "The entire flower bed on Carrer Nou de Sant Francesc  needs watering! The plants are dying out due to the immense heat.", location: "Carrer Nou de Sant Francesc 1, Barcelona", user: User.all.sample, category: Category.all[0])
report.photo.attach(io: file, filename: "plants.jpg", content_type: 'image/jpg')
puts "8"
file = URI.open("https://www.clubbookers.com/wp-content/uploads/2017/05/Pacha-Barcelona.jpg")
...
puts "9"
file = URI.open("https://static.timesofisrael.com/www/uploads/2018/11/iStock-938520600-e1542280189799.jpg")
...
puts "10"
file = URI.open("https://www.swarco.com/sites/default/files/public/hero/image/2018-10/cyclingpath_header.jpg")
report = Report.create!(title:"Bicycle path", description: "We need a bicycle path in our neighbourhood! The children are forced to drive in the heavy traffic making it extremely dangerous!", location: "Passeig de Picasso 40, Barcelona", user: User.all.sample, category: Category.all[6])
...
report = Report.create!(title:"Parking situation", description: "The parking situation on Carrer de Verdi is terrible. It is close to impossible to find a spot to park. The city should construct more parking spaces!", location: "Carrer de Verdi 2, Barcelona", user: User.all.sample, category: Category.all[6])
...
puts "13"
file = URI.open("https://s7d2.scene7.com/is/image/TWCNews/12-11-2019_Daytona_Beach_Homeless_Shelter")
...
puts "14"
file = URI.open("https://img.blogs.es/smartcitylab/wp-content/uploads/2019/07/vienna-smart-traffic-lights.jpg")
report = Report.create!(title:"Defect traffic light", description: "There is a defect traffic light on Carrer dels Almogàvers. It should be fixed asap in order to make the traffic safer.", location: "Carrer dels Almogàvers 122, Barcelona", user: User.all.sample, category: Category.all[3])
report.photo.attach(io: file, filename: "traffic-light.jpg", content_type: 'image/jpg')
puts "15"
file = URI.open("https://i.pinimg.com/originals/77/3c/db/773cdbc54faad54f1954648a73e921cd.jpg")
report = Report.create!(title:"Soccer pitch", description: "It would be great to have a public soccer pitch in Poblenou. There is free space in the park which could be used.", location: "Carrer de Zamora 78, Barcelona", user: User.all.sample, category: Category.all[0])
report.photo.attach(io: file, filename: "soccer.jpg", content_type: 'image/jpg')
puts "16"
file = URI.open("https://i.pinimg.com/originals/a1/48/9d/a1489dffc6594d0014a692ec0ef99006.jpg")
